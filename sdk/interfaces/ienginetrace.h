...
public:
	...
	CTraceFilterSkipTwoEntities( const IHandleEntity* pSkipEntity )
		: pSkip( pSkipEntity ), iTraceType( TRACE_EVERYTHING ), checkCallback( *reinterpret_cast< FilterCallbackFn*>( Displacement::Sigs.TraceFilterSkipTwoEntities ) ) { }
	...
	bool ShouldHitEntity( IHandleEntity* pHandleEntity, int fContentsMask ) override {
		...
		// otherwise skip entity if given
		return pHandleEntity != pSkip && pHandleEntity != pSkip2;
	}
